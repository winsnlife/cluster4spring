 ...
 * endpoints. This implementation simply selects the endpoint with minimal last access time.
 *
...
public class CopyOfLastAccessTimeEndpointSelectionPolicy<E extends Endpoint<SI>, SI extends ServiceMoniker>
        implements EndpointSelectionPolicy<E, SI> {
    protected static final Log fLog = LogFactory.getLog(CopyOfLastAccessTimeEndpointSelectionPolicy.class);

    public CopyOfLastAccessTimeEndpointSelectionPolicy() {
    }

    /**
     * Selects endpoint for invocation from the given list of endpoints based on value of
     * last access time for every endpoint. Endpoint with minimal value is selected.
     *
    ...
    public E selectServiceEndpoint(List<E> aEndpoints)
        ...
        // try to determine endpoint that was not accessed too long
        E lastAccessedEndpoint = null;
        long lastAccessTime = Long.MIN_VALUE;
        String previousEndpointURL = null;

        for (E endpoint : aEndpoints) {
            long endpointTime = endpoint.getLastAccessTime();
            String endpointURL = endpoint.getServiceInfo().getServiceURL();
            if (lastAccessedEndpoint == null) {
                lastAccessedEndpoint = endpoint;
                lastAccessTime = endpoint.getLastAccessTime();
                previousEndpointURL = endpointURL;
            } else if (lastAccessTime > endpointTime) {
                if (previousEndpointURL != endpointURL) {
                    lastAccessTime = endpointTime;
                    lastAccessedEndpoint = endpoint;
                    previousEndpointURL = endpointURL;
                }
            }
